...
export default function SmartWatches() {
    ...
    const fetch = () => {
        axios.get("http://localhost:8083/items/type?type=Smart watches")
            .then((res) => {
                ...
                return (setSmartWatches(res.data))
            }).catch((error) => {
                setError(true);
                if (error.response.data === undefined) {
                    setErrorMessage("Something went wrong")
    ...
    return (
        <div className='container-fluid'>
            <h2 id='Smart-watches' className='dark'>Smart Watches  <i className="bi bi-smartwatch" style={{ fontFamily: "fontAwesome" }}></i></h2>
            {fetchDone ?

                smartWatches.length == [] || !smartWatches.map(e => { e.itemType.toLowerCase().includes("Smart watches".toLowerCase()) }) ?
                    <div className='container-fluid justify-content-center text-center'>
                        <h1>No Items Found !</h1>
                    </div>
                    :
                    <div className='container-fluid justify-content-center text-center' id="back-card-bg-smartWatches">
                        <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4 gap-4 justify-content-center text-center">
                            {smartWatches.map(e => {
                                if (e.itemType.toLowerCase().includes("Smart watches".toLowerCase())) {
                                    i++;
                                    if (i <= 4) {
                                        return (
                        ...
                        <Link className='btn btn-info' to={'/viewmore/Smart watches'}>View More Smart watches Items....</Link><br></br>
                        &nbsp;
                    </div>
                :
