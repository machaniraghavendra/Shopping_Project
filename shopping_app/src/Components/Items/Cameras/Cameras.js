...
export default function Cameras() {
  ...
  const fetch = () => {
    axios.get("http://localhost:8083/items/type?type=Cameras")
      .then((res) => {
        if (res.status == "200") {
          setfetchDone(true)
        }
        return (setCameras(res.data))
      }).catch((error) => {
        setError(true);
  ...
  return (
    <div className='container-fluid'>
      <h2 id='Cameras' className='dark'>Cameras  <i className="bi bi-camera" style={{ fontFamily: "fontAwesome" }}></i></h2>
      {fetchDone ?
        Cameras.length == [] || !Cameras.map(e => { e.itemType.toLowerCase().includes("Cameras".toLowerCase()) }) ?
          <div className='container-fluid justify-content-center text-center'>
            <h1>No Items Found !</h1>
          </div>
          :
          <div className='container-fluid justify-content-center text-center' id="back-card-bg-Cameras">
            <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4 gap-4 justify-content-center text-center">
              {Cameras.map(e => {
                if (e.itemType.toLowerCase().includes("Cameras".toLowerCase())) {
                  i++;
                  if (i <= 4) {
                    return (
            ...
            <Link className='btn btn-info' to={'/viewmore/Cameras'}>View More Camera Items....</Link><br></br>
            &nbsp;
          </div>
        :
