...
export default function MusicalInstruments() {
  ...
  const fetch = () => {
    axios.get("http://localhost:8083/items/type?type=Musical Instruments")
      .then((res) => {
        if (res.status == "200") {
          setfetchDone(true)
        }
        return (setmusicalInstruments(res.data))
      }).catch((error) => {
        setError(true);
  ...
  return (
    <div className='container-fluid'>
      <h2 id='musicalInstruments' className='dark'>Musical Instruments  <i className="fa-solid fa-guitar" style={{ fontFamily: "fontAwesome" }}></i></h2>
      {fetchDone ?
        musicalInstruments.length == [] || !musicalInstruments.map(e => { e.itemType.toLowerCase().includes("Musical Instruments".toLowerCase()) }) ?
          <div className='container-fluid justify-content-center text-center'>
            <h1>No Items Found !</h1>
          </div>
          :
          <div className='container-fluid justify-content-center text-center' id="back-card-bg-musicalInstruments">
            <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4 gap-4 justify-content-center text-center">
              {musicalInstruments.map(e => {
                if (e.itemType.toLowerCase().includes("Musical Instruments".toLowerCase())) {
                  i++;
                  if (i <= 4) {
                    return (
            ...
            <Link className='btn btn-info' to={'/viewmore/Musical Instruments'}>View More Musical Items....</Link><br></br>
            &nbsp;
          </div>
        :
