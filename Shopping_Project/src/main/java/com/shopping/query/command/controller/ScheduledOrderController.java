...
public class ScheduledOrderController {
     ...
     @GetMapping("/order/{scheduledId}")
     public ResponseEntity<Object> getScheduledOrderDetailsById(@PathVariable("scheduledId") UUID scheduledId) {
          var response = scheduledOrderService.getScheduledOrderDetailsByUuid(scheduledId);
          return response == null ? ResponseEntity.badRequest().body("ScheduledId is not valid: " + scheduledId) : ResponseEntity.ok(response);
     }
}
